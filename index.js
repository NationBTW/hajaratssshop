...
client.on("message", async(message) =>{
 ...
 ${prefix}setstaff - set staff roles.`)
  ...
  if (command == prefix + `add`) {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: "RED" } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      try {
        channel.updateOverwrite(member.user, {
          ...
          message.lineReply({ embed: { description: `${member} has been successfully added to ${channel}`, color: "RED" } });
        ...
        channel.updateOverwrite(member.user, {
          VIEW_CHANNEL: false,
           ...
           let log_embed = new Discord.MessageEmbed()
            ...
            .setFooter(message.guild.name, message.guild.iconURL())
          channelLog(log_embed)
          message.lineReply({ embed: { description: `Successfully delete ${member} from ${channel}`, color: "RED" } });
        });
      }
      catch (e) {
        return message.channel.send(`An error has occurred, please try again!`);
      }
    }
  }
  if (command == prefix + 'delete') {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: "RED" } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      message.lineReply({ embed: { description: `Your order is executed after 5 seconds, and it will be closed`, color: "RED" } })
      setTimeout(async () => {
        let log_embed = new Discord.MessageEmbed()
            .setTitle(`Ticket Deleted`)
            .addField(`Ticket number`, `${await db.get(`reopened-ticket`).count}`)
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      setTimeout(async () => {
        try {
          ...
          channel.setName(`closed-${(await db.get(`ticket-closed`))}`)
          let log_embed = new Discord.MessageEmbed()
  ...
  if (command == prefix + 'open') {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: "RED" } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      let msg = await message.lineReply({ embed: { description: `Your order is executed after 5 seconds`, color: "RED" } })
      setTimeout(async () => {
        try {
          msg.delete()
          ...
          channel.setName(`reopened-ticket`)
          let log_embed = new Discord.MessageEmbed()
  ...
  if (command == prefix + 'rename' || command == prefix + 'setname') {
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      let args = message.content.split(' ').slice(1).join(' ');
      if (!args) return message.lineReply({ embed: { description: `Please select the name you want for the ticket`, color: "RED" } })
  ...
  if (command == prefix + 'setstaff'){
    ...
    if (args.length != 2) return message.lineReply({ embed: { description: `Please provide an Admin role id, *then* a Mod role id with this command! `, color: "RED" } })
    if (message.mentions.roles.length < 2 && !Number(args[0]) && !Number(args[1])) return message.lineReply({ embed: { description: `Please mention an Admin role (or iD) first, *then* a Mod role (or iD) with this command! `, color: "RED" } })
  ...
  if (command == prefix + 'setchannels'){
    if (!message.member.hasPermission('ADMINISTRATOR')) return message.lineReply(`:x: This command requires \`ADMINISTRATOR\` permission.`);
    if (args.length != 2) return message.lineReply({ embed: { description: `Please mention a channelid, *then* a categoryid with this command! `, color: "RED" } })
    if (message.mentions.roles.length < 2 && !Number(args[0]) && !Number(args[1])) return message.lineReply({ embed: { description: `Please mention an Log Channel (or iD), *then* a Category (or iD) with this command! `, color: "RED" } })
  ...
  if (command == prefix + 'send' || command == prefix + 'ticket') {
    ...
    if (!sfats || sfats === null) return message.lineReply({ embed: { description: `This server needs to set up their staff roles first! \`${prefix}setstaff\``, color: "RED" } })
    if (!sfas || sfas === null) return message.lineReply({ embed: { description: `This server needs to set up their ticket channels first! \`${prefix}setchannels\``, color: "RED" } })
    let idd = randomstring.generate({ length: 20 })
    let args = message.content.split(' ').slice(1).join(' ');
    if (!args) args = `Support`
    ...
    let embed = new Discord.MessageEmbed()
      .setTitle(args)
      .setDescription("To create a ticket, open the menu below.")
...
client.on('clickMenu', async (button, message, client) => {
  ...
  if (await db.get(`tickets_${button.id}_${button.message.guild.id}`)) {
    ...
    if (button.values[0] === "Purchase") { // Admins +
      button.guild.channels.create(`ticket-${count}`, {
        ...
        ], parent: (await db.get(`Channels_${button.message.guild.id}.Cat`)), position: 1, topic: `Ticket By: <@!${button.clicker.user.id}>`, reason: "All rights reserved to Visa2Code"
      }).then(async channel => {
        channel = channel
  ...
  To close this ticket, interact with 🔒`)
        ...
        if (button.values[0] === "General"){ // help +
          button.guild.channels.create(`ticket-${count}`, {
            permissionOverwrites: [
              {
                id: button.guild.roles.everyone,
          ...
          await button.reply.send(`Your order is executed after 5 seconds, and it will be closed`, true)   
            ...
            setTimeout(async () => {
              try {
                ...
                return button.channel.send(`An error has occurred, please try again!`);
              }
            }, 4000)
          }
        })
